...
/**
 * Controls the Swerve Modules using PercentOutput or Velocity for the drive motors and Position PID
 * for the angle motors. The left joystick controls translation (velocity direction and magnitude)
 * The right joystick's X axis controls rotation (angular velocity magnitude)
 *
 * <p>'back' is defined as closest to the battery 'left' is defined as left when standing at the
...
public class SwerveManualHeadingControl extends IndefiniteCommand {
  ...
  private static boolean counterClockwisePressed,
  ...
  public SwerveManualHeadingControl(
      HSSwerveDrivetrain drivetrain,
      PIDController headingController,
      HSGamepad gamepad,
      HSJoystickButton turnClockwise,
      HSJoystickButton turnCounterClockwise,
      int driveVelocitySlot,
      int anglePositionSlot,
  ...
  public SwerveManualHeadingControl(
      ...
      double maxRotationVelocity,
      double outputMultiplier,
      HSJoystickButton slowMode,
      double slowModeMultipier) {
    this(
        drivetrain,
        headingController,
        gamepad,
  ...
  public void execute() {
    ...
    turnMagnitude =
        -1 * headingController.calculate(drivetrain.getPigeon().getFusedHeading(), headingAngle);
    ...
    translateX *=
        outputMultiplier
            * maxDriveVelocity
            * ((hasSlowMode && slowMode.get()) ? slowModeMultiplier : 1);
    translateY *=
        outputMultiplier
            * maxDriveVelocity
            * ((hasSlowMode && slowMode.get()) ? slowModeMultiplier : 1);
    ...
    if (counterClockwiseFlag) headingAngle = drivetrain.getPigeon().getFusedHeading() - 130;

    clockwiseFlag = clockwisePressed && !turnClockwise.get();
    clockwisePressed = turnClockwise.get();

    if (clockwiseFlag) headingAngle = drivetrain.getPigeon().getFusedHeading() + 130;

    if (counterClockwisePressed) turnMagnitude = -0.7 * outputMultiplier * maxRotationVelocity;
    else if (clockwisePressed) turnMagnitude = 0.7 * outputMultiplier * maxRotationVelocity;
    ...
    if (centricity) {
      speeds =
          ChassisSpeeds.fromFieldRelativeSpeeds(
              translateX,
              translateY,
              (headingFlag || flag) ? turnMagnitude : 0,
