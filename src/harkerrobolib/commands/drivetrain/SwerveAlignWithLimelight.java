...
/**
 * Aligns the Robot with the target, using the Limelight's tX to turn towards the target and its
 * thor to get close enough to the target
 *
...
public class SwerveAlignWithLimelight extends CommandBase {
  ...
  public void initialize() {
    drivetrain.applyToAllDrive(
        (falcon) -> falcon.selectProfileSlot(driveVelocitySlot, Constants.PID_PRIMARY));
    drivetrain.applyToAllDrive((falcon) -> falcon.configClosedloopRamp(driveRampRate));

    drivetrain.applyToAllAngle(
        (talon) -> talon.selectProfileSlot(anglePositionSlot, Constants.PID_PRIMARY));
  ...
  public void execute() {
    ...
    double turn =
        txController.calculate(Limelight.getTx(), txController.getSetpoint())
            * maxRotationVelocity
            * (DriverStation.isAutonomous() ? -1 : 1);

    double translateX =
        MathUtil.mapJoystickOutput(driverGamepad.getLeftX(), Constants.JOYSTICK_DEADBAND);
    double translateY =
        MathUtil.mapJoystickOutput(driverGamepad.getLeftY(), Constants.JOYSTICK_DEADBAND);

    double turnManual =
        -3 * MathUtil.mapJoystickOutput(driverGamepad.getRightX(), Constants.JOYSTICK_DEADBAND);
    ...
    ChassisSpeeds speeds =
        ChassisSpeeds.fromFieldRelativeSpeeds(
            translateX,
            translateY,
            Limelight.isTargetVisible() ? turn : turnManual,
    ...
    drivetrain.setDrivetrainVelocity(
        moduleStates[0], moduleStates[1], moduleStates[2], moduleStates[3], false, false);
  }
