...
/**
 * Controls the drivetrain to follow a Motion Profile Trajectory using Odometry control, mostly
 * copied from WPILib's SwerveControllerCommand
...
public class SwerveDriveWithOdometry extends CommandBase {
  ...
  public SwerveDriveWithOdometry(
  ...
  public void initialize() {
    ...
    Rotation2d initialRotation =
        trajectory.getInitialPose().getRotation().minus(Rotation2d.fromDegrees(0));
    ...
    while (System.currentTimeMillis() - initialTime < TIMEOUT && !isAtSepoint) {
      isAtSepoint =
          Math.abs(drivetrain.getTopLeft().getAngleErrorDegrees(initialRotation.getDegrees()))
                  < ALLOWABLE_ERROR
              && Math.abs(
                      drivetrain.getTopRight().getAngleErrorDegrees(initialRotation.getDegrees()))
                  < ALLOWABLE_ERROR
              && Math.abs(
                      drivetrain.getBackLeft().getAngleErrorDegrees(initialRotation.getDegrees()))
                  < ALLOWABLE_ERROR
              && Math.abs(
                      drivetrain.getBackRight().getAngleErrorDegrees(initialRotation.getDegrees()))
                  < ALLOWABLE_ERROR;
  ...
  public void execute() {
    ...
    double targetXVel =
        xController.calculate(
            pose.get().getTranslation().getX(), desiredPose.getTranslation().getX());

    double targetYVel =
        yController.calculate(
            pose.get().getTranslation().getY(), desiredPose.getTranslation().getY());
    ...
    double targetAngularVel =
        thetaController.calculate(
            Math.toRadians(-drivetrain.getPigeon().getFusedHeading()), heading.getRadians());
