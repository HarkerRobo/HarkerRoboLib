...
public class SwerveManual extends IndefiniteCommand {
  ...
  public void execute() {
    ...
    if (pigeonFlag && turnMagnitude == 0) { // If there was joystick input but now there is not
      double velocityHeadingMultiplier =
          Math.abs(turnVel) > TURN_VEL_THRESHOLD
              ? HIGH_VELOCITY_HEADING_MULTIPLIER
              : LOW_VELOCITY_HEADING_MULTIPLIER;

      // account for momentum when turning
      pigeonAngle =
          currentPigeonHeading
              + turnVel * velocityHeadingMultiplier
              + turnAccel * ACCELERATION_HEADING_MULTIPLIER;
    }
