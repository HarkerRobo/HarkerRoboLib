...
/**
 * Simulates the drivetrain subsystem on the robot. A Swerve Drivetrain contains four SwerveModules.
 *
 * <p>Acronyms: TL: Top Left TR: Top Right BL: Back Left BR: Back Right
 *
...
public abstract class HSSwerveDrivetrain extends SubsystemBase {
   ...
   *
   * <p>Initializes SwerveModules with inverts for drive and angle motors and sensor phases, resets
   * the encoders to the offset position and zeroes the angle motors so that all modules are facing
   * forward at 90 degrees, zeroes the pigeon to 90 degrees, and initializes SwerveDriveKinematics
   * and Odometry.
   */
  public HSSwerveDrivetrain(
      SwerveModule[] modules, HSPigeon pigeon, double chassisWidth, double chassisLength) {
    topLeft = modules[0];
    topRight = modules[1];
    backLeft = modules[2];
    backRight = modules[3];
    ...
    odometry =
        new SwerveDriveOdometry(
            kinematics,
            new Rotation2d(pigeon.getFusedHeading()),
            new Pose2d(new Translation2d(), Rotation2d.fromDegrees(90)));

    Pose2d initialPose =
        new Pose2d(new Translation2d(), Rotation2d.fromDegrees(pigeon.getFusedHeading()));
  ...
  public static SwerveModule[] initSwerveModules(
      ...
      int[] angleIds,
      int[] offsets,
      TalonFXInvertType[] driveInverts,
      boolean[] driveSensorPhases,
      boolean[] angleInverts,
      boolean[] angleSensorPhases,
  ...
  /**
   * Updates odometry periodically based on the pigeon heading and the states of the SwerveModules.
   */
  @Override
  ...
  public void setupVelocityPID(
      int slot, double kF, double kP, double kI, double kD, double rampRate) {
  ...
  /**
   * Sets the velocity and angle of the drivetrain motors based on the properties of the passed
   * SwerveModuleStates, with feedforward set to 0, percent output as false, and motion profile as
   * true.
   */
  ...
  public void setDrivetrainVelocity(
      SwerveModuleState tl,
      SwerveModuleState tr,
    ...
    setSwerveModuleVelocity(
        topLeft,
        tlOutput,
        convertAngle(topLeft, tl.angle.getDegrees()),
        isPercentOutput,
        isMotionProfile);
    setSwerveModuleVelocity(
        topRight,
        trOutput,
        convertAngle(topRight, tr.angle.getDegrees()),
        isPercentOutput,
        isMotionProfile);
    setSwerveModuleVelocity(
        backLeft,
        blOutput,
        convertAngle(backLeft, bl.angle.getDegrees()),
        isPercentOutput,
        isMotionProfile);
    setSwerveModuleVelocity(
        backRight,
        brOutput,
        convertAngle(backRight, br.angle.getDegrees()),
        isPercentOutput,
        isMotionProfile);
  }

  public void setSwerveModuleVelocity(
      SwerveModule module,
      double output,
      double angle,
      boolean isPercentOutput,
  ...
  /**
   * Converts the target angle from the desired Vectors into the actual angle for the motors to
   * hold. Angle modification for Field sensitive drive should have already been handled.
   *
   * <p>Steps:
   *
   * <p>1. Subtract 90 degrees. 0 degrees on the Joysticks and desired Vectors points to the right
   * (positive x axis) while 0 degrees on the robot points forward (positive y axis). The
   * subtraction deals with this offset. 2. Increase/Decrease the targetAngle by 360 degrees until
   * it is within +- 180 degrees of the current angle
   *
