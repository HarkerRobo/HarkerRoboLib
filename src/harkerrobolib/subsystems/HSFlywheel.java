...
/**
 * Represents a general Flywheel shooter with a master talon and a follower talon, and optional
 * solenoid.
 *
 * @author Aimee Wang
 * @author Kunal Jain
...
public abstract class HSFlywheel<Motor extends HSMotorController> extends SubsystemBase {
  ...
  /**
   * Constructs a flywheel with master and follower motorcontrollers and a solenoid. Must pass in
   * stall current and stall velocity.
   *
  ...
  /**
   * Sets up the master and follower motorcontrollers by resetting defaults, setting master's sensor
   * phase, setting inverts, setting up velocity PID, set voltage compensation.
   *
   * @param sensorPhase
   * @param masterInvert
  ...
  public void setupFlywheel(
      boolean sensorPhase,
      boolean masterInvert,
      boolean followerInvert,
      int velocitySlot,
      Gains constants,
      double voltageComp,
  ...
  public void toggleSolenoid() {
    if (solenoid != null) {
      solenoid.set(
          (solenoid.get() == DoubleSolenoid.Value.kForward) ? Value.kReverse : Value.kForward);
    }
  }
  ...
  public void spinShooterPercentOutput(double percentOutput) {
    ...
    if (percentOutput == 0) master.set(ControlMode.Disabled, 0);
    else master.set(ControlMode.PercentOutput, percentOutput);
  }
