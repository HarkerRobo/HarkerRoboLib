...
public abstract class HSIntake<Motor extends HSMotorController> extends SubsystemBase {
  ...
  public HSIntake(Motor masterMotor, Motor followerMotor, DoubleSolenoid solenoid) {
    this.masterMotor = masterMotor;
    this.followerMotor = followerMotor;
   ...
   *
   * @param signum sign for output of master intake with given direction (must be negated for
   *     follower)
   */
  ...
  public void setSensorPhases(boolean masterSensorPhase, boolean followerSensorPhase) {
    ...
    if (followerMotor != null) followerMotor.setSensorPhase(followerSensorPhase);
  }
  ...
  public void invertTalons(boolean masterInverted, boolean followerInverted) {
    ...
    if (followerMotor != null) followerMotor.setInverted(followerInverted);
  }
  ...
  public void setCurrentLimits(double peakTimeSeconds, double peakCurrent, double contCurrent) {
    masterMotor.configStatorCurrentLimit(
        new StatorCurrentLimitConfiguration(true, contCurrent, peakCurrent, peakTimeSeconds));
  }

  /**
   * Sets wheels to the same output and direction.
   *
   * @param output value from controller that sets output for master and follower wheels to same
   *     value
  ...
  public boolean isStalling(double minCurrentDraw, double jammedVelocity) {
    return (masterMotor.getOutputCurrent() > minCurrentDraw
            && masterMotor.getSelectedSensorVelocity() < jammedVelocity)
        || (followerMotor != null
