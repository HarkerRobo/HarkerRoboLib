...
public abstract class HSElevator extends SubsystemBase {
     ...
     * @param softLimitPosition where soft limit (position on elevator to slow down in downward direction) is located
     * @param maxLessOutputFactor upper bound of output factor when speed percent is less than slow down percent
     * @param minLessOutputFactor lower bound of output factor when speed percent is less than slow down percent
    ...
    public void set(double desiredSpeed) {
        if (LOOP_INDEX != -1) {
            if (Math.abs(desiredSpeed) > 0) {
                ...
                if (rampDown && reversePastLimit && Math.abs(currVelocity) / MAX_SPEED < SLOW_DOWN_PERCENT) {
                    outputFactor = MathUtil.map(distFromSoft, 0, SOFT_LIMIT_POSITION, MIN_LESS_OUTPUT_FACTOR,
                            MAX_LESS_OUTPUT_FACTOR);
                } else if (rampDown && reversePastLimit) {
                    outputFactor = MathUtil.map(distFromSoft, 0, SOFT_LIMIT_POSITION, MIN_MORE_OUTPUT_FACTOR,
