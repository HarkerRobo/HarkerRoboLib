...
/**
 * Represents a general Drivetrain subsystem with two master motor controllers and two optional
...
public abstract class HSDiffDrivetrain<Motor extends HSMotorController> extends SubsystemBase {
  ...
  /**
   * Constructs a new Drivetrain subsystem with two master controllers, two follower controllers,
   * and a Pigeon.
   *
   ...
   */
  public HSDiffDrivetrain(
      Motor leftMaster,
      Motor rightMaster,
      IMotorController leftFollower,
   ...
   */
  public HSDiffDrivetrain(
      Motor leftMaster, Motor rightMaster, HSPigeon pigeon, double wheelDiameter) {
   ...
   * @param continuousLimit the continuous limit, or the one which is active after the conclusion of
   *     the peak limit.
   */
  public void setCurrentLimit(double peakCurrent, double peakTime, double continuousLimit) {
    Consumer<Motor> currentLimit =
        (talon) -> {
          talon.configStatorCurrentLimit(
              new StatorCurrentLimitConfiguration(true, continuousLimit, peakCurrent, peakTime));
        };
  ...
  public void invertTalons(
      boolean leftMasterInverted,
      boolean rightMasterInverted,
      boolean leftFollowerInverted,
      boolean rightFollowerInverted) {
  ...
  public void configBothFeedbackSensors(FeedbackDevice fd, int pidLoop) {
    getLeftMaster().configSelectedFeedbackSensor(fd, pidLoop);
    getRightMaster().configSelectedFeedbackSensor(fd, pidLoop);
  }

  /** Prints the current output percentage to the motors to SmartDashboard. */
  public void printMotorOutputPercentage() {
    SmartDashboard.putNumber(
        "Left Talon Output Percentage", getLeftMaster().getMotorOutputPercent());
    SmartDashboard.putNumber(
        "Right Talon Output Percentage", getRightMaster().getMotorOutputPercent());
  }
  ...
  public void printClosedLoopError(int pidLoop) {
    SmartDashboard.putNumber(
        "Left Talon Closed Loop Error " + (pidLoop == 0 ? "Primary" : "Auxiliary"),
        getLeftMaster().getClosedLoopError(pidLoop));
    SmartDashboard.putNumber(
        "Right Talon Closed Loop Error " + (pidLoop == 0 ? "Primary" : "Auxiliary"),
        getRightMaster().getClosedLoopError(pidLoop));
  }
  ...
  public void printSensorPositions(int pidLoop) {
    SmartDashboard.putNumber(
        "Left Talon Position " + (pidLoop == 0 ? "Primary" : "Auxiliary"),
        getLeftMaster().getSelectedSensorPosition(pidLoop));
    SmartDashboard.putNumber(
        "Right Talon Position" + (pidLoop == 0 ? "Primary" : "Auxiliary"),
  ...
  public void arcadeDrivePercentOutput(double speed, double turn) {
    double divisor =
        Math.max(
            1, Math.max(Math.abs(speed + Math.pow(turn, 2)), Math.abs(speed - Math.pow(turn, 2))));
  ...
  public void arcadeDriveVelocity(double speed, double turn) {
    ...
    double rightOutputBase = speed - turn * Math.abs(turn);
    rightMaster.set(ControlMode.Velocity, rightOutputBase / divisor);
  }
}
