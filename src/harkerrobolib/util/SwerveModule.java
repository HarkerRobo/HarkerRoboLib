...
/**
 * A swerve module on the drivetrain. A swerve module contains one Falcon 500 to rotate the wheel
 * and one TalonSRX/775pro combo to angle the wheel.
 *
 * @author Jatin Kohli
...
public class SwerveModule {
   ...
   * @param driveSensorPhase Whether or not to invert the sensor phase of the drive motor
   ...
   * @param angleSensorPhase Whether or not to invert the sensor phase of the angle motor
   *     controller's encoder.
   * @param wheelDiameter The wheel diameter (in inches) of the drive wheels.
   * @param gearRatio The gear ratio from the drive encoder to the drive wheel (6 for a 6:1, where 6
   *     revs of the encoder mean 1 rev of the drive wheel).
   * @param offset The offset (in absolute encoder ticks) for the angle encoder to have.
   */
  public SwerveModule(
      int driveId,
      TalonFXInvertType invertDrive,
      boolean driveSensorPhase,
      int angleId,
      boolean invertAngle,
      boolean angleSensorPhase,
    ...
    angleMotor.setSelectedSensorPosition(
        (angleMotor.getSensorCollection().getPulseWidthRiseToFallUs() - TELEOP_OFFSET) / 4);
  }
  ...
  public void driveFalconInit(TalonFX falcon) {
    ...
    falcon.configSelectedFeedbackSensor(
        TalonFXFeedbackDevice.IntegratedSensor, Constants.PID_PRIMARY, Constants.DEFAULT_TIMEOUT);
  ...
  /**
   * @param isDrive True if the configuration is to be applied to the drive motor, false if for the
   *     angle motor
  ...
  public void configCurrentLimit(boolean isDrive, int continuous, int peak, int duration) {
    if (isDrive)
      driveMotor.configStatorCurrentLimit(
          new StatorCurrentLimitConfiguration(true, continuous, peak, duration));
    else {
  ...
  /**
   * Sets the drive output of the swerve module in either percent output or velocity in feet per
   * second.
   *
   * @param output The output of the swerve module, with units corresponding to isPercentOutput.
   * @param isPercentOutput True if the output's units are a percentage of maximum output, false if
  ...
  /**
   * Sets the module's output to have the desired angle and output. 90 degree optimizations are
   * performed if needed.
   *
   * @param targetAngle The angle in degrees for the module's angle motor to have. Zero degrees is
   *     in the positive x direction.
   * @param output The output for the module's drive motor to have, with units corresponding to
   *     isPercentOutput.
   * @param isPercentOutput True if the output's units are a percentage of maximum output, false if
   *     the units are meters per second.
   * @param isMotionProfile True if the robot is currently in a motion profile and should not
   *     perform 90 degree optimizations, false if otherwise.
   */
  public void setAngleAndDriveVelocity(
      double targetAngle, double output, boolean isPercentOutput, boolean isMotionProfile) {
  ...
  public double getAngleDegrees() {
    return angleMotor.getSelectedSensorPosition()
        * 360.0
        / ENCODER_TICKS; // Convert encoder ticks to degrees
  }
  ...
  /**
   * Returns the current SwerveModuleState of the module, which contains the angle (Rotation2d) and
   * speed (m/s) of the module.
   */
